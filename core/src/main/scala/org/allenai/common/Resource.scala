...
object Resource {
  /** A method for managing a simple resource.  After running a computation, the resource will be
    * closed and the computation will be returned.
    *
    * More complicated resource management may warrant a dependency on "com.jsuereth" % "scala-arm",
    * but this simple method handles most cases.
    *
  ...
  /** A method for managing a Source.  After running a computation, the Source will be closed and
    * the computation will be returned.
    *
    * This method exists because in Scala 2.11 Source doesn't extend java.io.Closeable.
    *
    * @param resource the source to auto-close
    * @param f a computation involving the supplied resource
    * @return the result of the computation over the resource
    */
  def using[B](resource: Source)(f: Source => B): B = {
    require(resource != null, "The supplied resource was null.")
    try {
      f(resource)
    } finally {
