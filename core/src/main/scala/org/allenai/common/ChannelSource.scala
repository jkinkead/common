...
/** A class that provides seekability to an interface similar to scala.io.Source. This is required
  ...
  * ISO-8859-1
  */
class ChannelSource(inFile: FileChannel, bufferSize: Int = 8192)(implicit codec: Codec)
  ...
  /** Iterator class for reading the file linewise. This keeps a byte array that's a mirror of the
    * ByteBuffer in the wrapping ChannelSource. This is scanned for encoded newlines and / or
  ...
  private[common] class LineIterator extends Iterator[String] {
    ...
    private[common] def ensureFullBuffer(): Boolean = {
      // If we're at the end of our lineBuffer, we need to read more from ChannelSource.
      if (index == limit) {
    ...
    override def next(): String = {
      ...
      // Update our index + limit, in case the underlying ChannelSource has moved since we were last
      // iterated on.
...
object ChannelSource {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def timeChannelSource(): Unit = {
      println("Timing ChannelSource -")
      var startTime = System.currentTimeMillis
      val inFile = FileChannel.open(java.nio.file.Paths.get(wikip))
      val channelSource = new ChannelSource(inFile)
      println("Timing char iterator line count . . .")
      val charCount = channelSource count { _ == '\n' }
      println(s"ChannelSource charCount = $charCount")
      println(s"ChannelSource took ${System.currentTimeMillis - startTime} millis.")

      channelSource.position(0)
      startTime = System.currentTimeMillis
      println("Timing getLines line count . . .")
      val getLinesCount = channelSource.getLines().size
      println(s"ChannelSource getLinesCount = $getLinesCount")
      println(s"ChannelSource took ${System.currentTimeMillis - startTime} millis.")
    ...
    def timeSource(): Unit = {
      println("Timing scala.io.Source -")
      var startTime = System.currentTimeMillis
      var scalaSource = scala.io.Source.fromFile(wikip)
      println("Timing char iterator line count . . .")
      val charCount = scalaSource count { _ == '\n' }
      println(s"Scala Source charCount = $charCount")
      println(s"Scala Source took ${System.currentTimeMillis - startTime} millis.")
      scalaSource.close

      scalaSource = scala.io.Source.fromFile(wikip)
      startTime = System.currentTimeMillis
      println("Timing getLines line count . . .")
      val getLinesCount = scalaSource.getLines().size
      println(s"Scala Source getLinesCount = $getLinesCount")
      println(s"Scala Source took ${System.currentTimeMillis - startTime} millis.")
